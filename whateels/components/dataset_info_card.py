...
class SupportsAppState(Protocol):
    """Minimal contract required from a page model by this card.

    The card never touches page-specific behaviour: it only needs `.app_state`
    to propagate an edited attribute to the shared application state. Any page
    model exposing `app_state` (Home, Fitting, Quantification, ...) qualifies.
    """
...
def create_dataset_info_card(
    model: SupportsAppState,
    dataset: "Dataset",
    *,
    sizing_mode: str = "stretch_width",
    margin=0,
) -> InfoPanel:
    """Build the shared metadata card with editable acquisition values.

    This is THE single implementation of the "Dataset Information" card. It is used
    by the Home page plots and by the Fitting page Elemental tab, so an edit made in
    one page is written into `dataset.attrs` and propagated through `model.app_state`
    to every other page looking at the same dataset.

    `model` is intentionally typed as a structural protocol instead of a concrete
    page model: the card only reads `model.app_state`, so any page model exposing
    that attribute (HomePageModel, FittingModel, ...) can be passed in.
    """
