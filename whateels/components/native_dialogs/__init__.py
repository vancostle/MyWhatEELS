...
def open_native_file_dialog(accepted_file_types: Sequence[str] | None = None) -> str:
    ...
    if sys.platform == "win32":
        return open_windows_file_dialog(accepted_file_types)

    if sys.platform == "darwin":
        return open_macos_file_dialog(accepted_file_types)

    if sys.platform.startswith("linux"):
        return open_linux_file_dialog(accepted_file_types)
