...
print('Importing - Clustering Analysis panels for Spectrum Lines ')
...
class plotting_cluster():
    def __init__(self, ds,lite = False,given_colors = []):
        ...
        #We have to create the image of an specturm line
        '''
        self.original_im = hv.Image(self.ds.ElectronCount,kdims = ['y','Eloss'])\
            .opts(cmap = 'Greys_r',invert_axes=True,invert_yaxis=True,\
                frame_width = 900,frame_height = 125,yformatter = formatter2,\
                xaxis = None,toolbar = None)
        self.empty_spectrum =\
        ...
        self.original_im = hv.Image(self.ds.ElectronCount,kdims = ['y','Eloss'])\
            .opts(cmap = 'Greys_r',invert_axes=True,invert_yaxis=True,\
                frame_width = 1000,frame_height = 175,yformatter=formatter,\
                xformatter=formatter,ylabel = 'Line pixel #',\
                xaxis = None,toolbar = 'right')
        self.im = hv.Image(self.ds['labs'],kdims = ['y','Eloss'])\
            .opts(tools = ['hover'],cmap = self.colores[:self.long],framewise = True,\
                invert_axes=True,invert_yaxis=True,shared_axes= False,\
                ylabel = 'Line pixel #',xformatter=formatter,\
                frame_width = 1000,frame_height = 175,yformatter=formatter,\
                xaxis = None,toolbar = 'right',alpha = 0.5)

        self.mini_image = hv.Image(self.ds['labs'],kdims = ['y','Eloss'])\
            .opts(toolbar = None,cmap = self.colores[:self.long],framewise = True,\
                invert_axes=True,invert_yaxis=True,shared_axes= False,padding =0, border = 0,\
                frame_height = 30,frame_width = 275,xaxis = None,yaxis = None)

        #self.doble_tap_point = streams.DoubleTap(x = 0, y = 0, source = self.point_graph)
        self.doble_tap_image = streams.DoubleTap(x = self.ds.Eloss.values[0],y = 0, source = self.im)
        #The are the down/upper limits for hovering (xi,xf,yi,yf) - a tuple
        self.hov_lims = (self.ds.Eloss.data[0],self.ds.Eloss.data[-1],self.ds.y.data[0]-0.5,self.ds.y.data[-1]+0.5)
    ...
    def plot_curve(self,x,y):
        ...
        if x >= Ef or x <= Ei:
            curve = hv.Area([], 'Eloss', 'ElectronCount')\
                .opts(yformatter=formatter,frame_width = 1000)
            ...
            curve = hv.Area(self.ds['Centroid_pixel'].isel(x = 0,y = int(y0)))\
            .opts(color = col,yformatter=formatter,frame_width = 1000)
            curve.relabel('Tick formatters')
            return curve    
    ...
    def _create_dynamic_maps(self):
        ...
        .opts(frame_height = 275,framewise = True,\
            shared_axes=False,fill_alpha = 0.5,frame_width = 650,\
            yformatter = formatter)
        '''
        self.din2 = hv.DynamicMap(self.plot_curve,streams=[self.doble_tap_image])\
        .opts(frame_height = 275,framewise = True,\
            shared_axes=False,fill_alpha = 0.5,frame_width = 1000,\
            yformatter = formatter)
        self.over = self.hmap.overlay('cluster_number')\
        .opts(frame_height = 275,framewise = True,\
            frame_width = 1000,shared_axes=False,yformatter = formatter)
    ...
    *Available norms decide the norm to be applied
    ...
    def __init__(self,ds):
        ...
        self.im = hv.Image(self.ds.ElectronCount,kdims = ['y','Eloss'])\
            .opts(cmap = 'Greys_r',invert_axes=True,invert_yaxis=True,\
                frame_width = 1000,frame_height = 175,\
                ylabel = 'Line pixel #',xformatter=formatter,\
                xaxis = None,yformatter=formatter,toolbar = 'right')
        
        '''       
        self.im = hv.Image(self.ds.sum(dim = 'Eloss'))\
            .opts(cmap = 'Greys_r',aspect=1,invert_axes=True,\
        ...
        self.button_save = pn.widgets.Button(name = 'Save Run',\
            disabled=True,button_type='default',width = 325,height = 45,\
            margin = (20,15),align = 'center')
        self.button_save.on_click(self._callback_button_save)
        self.button_show_results = pn.widgets.Button(name = 'Show Selection',\
            disabled = True, button_type = 'default',width = 325,height = 45,margin = (20,15))
        ...
        self.button_type_curve = pn.Param(self.param['dynamic_curves_repr'],\
            ...
            width = 150,height = 35,align = 'center',margin = (5,35))
        ...
        self.button_type_curve[0].margin = 0
        #Point size widget
        ...
        '''
        self.im_default = hv.Image(self.ds_zero.ElectronCount,kdims = ['y','Eloss'])\
            .opts(cmap = 'Greys_r',invert_axes=True,invert_yaxis=True,\
                frame_width = 650,frame_height = 150,\
                xaxis = None,yaxis = None,toolbar = None)
        '''
        self.mini_image_default = hv.Image(self.ds_zero.ElectronCount,kdims = ['y','Eloss'])\
            .opts(cmap = 'magma',invert_axes=True,invert_yaxis=True,\
                xaxis = None,yaxis = None,toolbar = None,padding =0, border = 0,\
                frame_height = 30,frame_width = 275)
        self.curve_default = hv.Area(self.ds_zero.sum(['x','y']))\
            .opts(frame_height = 275,frame_width = 1000,xlabel='Electron energy loss [eV]',\
            ylabel = 'Electron Counts [a.u]',shared_axes = False,yformatter=formatter)
        ...
        self.df_widgets = pn.pane.DataFrame(df_initial,height = 75,width = 750,margin = (0,5,10,5))
        ...
        self.savname_wid = pn.Param(self.param['saving_name'],\
            ...
            show_labels = False,width = 650,margin = 0)
        self.savname_wid[0].placeholder = 'Enter File name for data saves'
        self.savname_wid[0].width = 650
    ...
    def _carry_on_clustering(self):
        ...
        shap = data.shape
        data_flat = data.reshape(shap[0]*shap[1],Esize)
        ...
        if self.cluster_tab.active == 1:
            ...
            if self.connectivity_matrix:
                ...
                if self.connectivity_on_normalized:
                    ...
                    connect = kneighbors_graph(data.reshape(shap[0]*shap[1],Esize),\
                        n_neighbors=self.number_of_neighbors_agg,\
                        include_self=False)
        ...
        #label_map = self.clustered.labels_.reshape(shap[0],shap[1])
        label_map = self.clustered.labels_.reshape(shap[0],)
        #self.label_map = label_map
        zero_label = np.zeros((Esize,shap[0]))
        zero_label[:] = label_map
        self.label_image = zero_label.transpose()
        try:
            self.centroides = self.clustered.cluster_centers_
        except:
            #In case of not having the method of extraction
            self.centroides = np.zeros((self.num_cluster,Esize))
        #Now we need the objects to be plotted
        mat_cents = np.zeros(shap)                        #mapping with the centroids per image/pixel
        cents = np.zeros((self.num_cluster,Esize))     #centroids signals
        cents_2 = np.zeros((self.num_cluster,Esize))
        for i in range(self.num_cluster):
            mat_cents[label_map == i] = data[label_map == i].mean(axis = (0))
            cents[i,:] = data[label_map == i].mean(axis = (0))
        ...
        coords= {'Eloss' : Eloss,  'y' : self.ds.y.values,\
            'x' : self.ds.x.values, 'label_number' : lab_numer},\
        ...
        self.spectra_placeholder.object = self.pt.din2
        self.sl_placeholder.object = self.im*self.pt.im
    ...
    def _change_button_style_2(self):
        if self.dynamic_curves_repr:
            self.type_curve = 'DynamicGraph'
            self.button_type_curve[0].button_type = 'success'
            self.button_type_curve[0].name = self.type_curve
            ...
            self.spectra_placeholder.object = self.pt.over
    ...
    def _callback_show_clusters(self,event):
        ...
        self.pt = plotting_cluster(ds = self.saves[self.runs_selection]\
            ['loading_info']['data'],given_colors = self.saves[self.runs_selection]\
            ['loading_info']['colors'])
        self.pt._create_dynamic_maps()
        #Now all the possible options for display
        #self.image_column[2][0].pop(0)
        self.sl_placeholder.object = self.im*self.pt.im
        ...
        self.visual_controls[5].object = '### None'
        self.visual_controls[5].style = {'color':'darkgrey'}
        self.visual_controls[1].object = '### {}'.format(self.runs_selection)
        self.visual_controls[1].style = {'color':'dodgerblue'}
    ...
    def _callback_button_cluster(self, event):
        ...
        self.visual_controls[5].object = '### NOT STORED'
        self.visual_controls[5].style = {'color':'red'}
        self.visual_controls[1].object = '### Current Run'
        self.visual_controls[1].style = {'color':'limegreen'}
    ...
    def _callback_button_store(self,event):
        ...
        if self.number_of_runs == 1:
            ...
            self.mini_image_placeholder.object = ima
        ...
        self.visual_controls[5].object = '### STORED'
        self.visual_controls[5].style = {'color':'limegreen'}
    ...
    def _callback_button_save(self,event):
        ...
        dss.attrs = [('Original_name',orig),('Algorithm',algo),('Parameters',algo_paras)]
        #One las modification
        dirname = '/'.join(self.saving_folder.split(r'\\')) #Just to have a common format
        save_place = '/'.join([dirname,fname])
    ...
    def create_panel(self):
        ...
        pn.Row(pn.pane.Markdown('#### Pre-Normalization',style = {'color':'white'},width = 140),\
            self.prenorm_button,width = 310,margin = (5,10)),\
        self.available_norms_wid,width = 350,height = 750,background = 'black')
        #button_column.margin = 0
        for el in button_column:
            el.margin = (5,10)
        #button_column[5].margin = (0,5)
        self.mini_image_placeholder = pn.pane.HoloViews(self.mini_image_default,\
            width = 275,margin = (15,15,5,15))
        select_run_pan = pn.Row(\
            pn.pane.Markdown('#### Run Selector',align = 'center',\
                style = {'color':'white'},width = 150),\
            self.mini_image_placeholder,self.run_buttons_widgets,\
            width = 1150,margin = (0,15))
        saves_lines = pn.Column(\
            pn.Row(pn.pane.Markdown('#### FileName',width = 100,style = {'color':'white'}),\
                self.savname_wid,height = 40,margin = 0,width = 750),\
            pn.Row(pn.pane.Markdown('#### FolderName',width = 100,style = {'color':'white'}),\
                self.savfold_wid,height = 40,margin = 0,width = 750),\
            width = 775,height = 80)
        saves_pan = pn.Row(saves_lines,self.button_save,width = 1130,margin = (0,15))
        current_selection_pan = pn.Column(select_run_pan,pn.Row(\
            self.df_widgets,self.button_show_results,\
            width = 560,margin = (0,15)),width = 1130,margin = (0,15))
        self.column_saves = pn.Column(\
            pn.pane.Markdown('### Run selection and saving HUB',\
                style = {'color':'white'},margin = (5,15,5,15),width = 900),\
            pn.layout.Divider(height = 5,margin = (-15,5,15,15)),\
            current_selection_pan,\
            pn.layout.Divider(height = 5,margin = (-15,5,15,15)),\
            saves_pan,\
            height = 350,width = 1150,background = 'grey')
        self.sl_placeholder = pn.pane.HoloViews(\
            self.im,width = 1050,margin = (10,10))
        self.spectra_placeholder = pn.pane.HoloViews(\
            self.curve_default,height = 330,width = 1050)
        #default_image_row.margin = (0,0,0,25)
        #self.button_save.margin = (0,10,0,15)
        self.button_show_results.margin = (0,10,0,15)
        self.visual_controls = pn.Row(\
            self.button_type_curve,\
            pn.pane.Markdown('### Results :',width = 120),\
            pn.pane.Markdown('### None',style={'color':'darkgrey'},width = 205),\
            pn.Spacer(height = 35,width = 15),\
            pn.pane.Markdown('### Run status:',width = 100),\
            pn.pane.Markdown('### None',style = {'color':'darkgrey'},width = 150),\
            width = 1050)
        self.visual_controls.margin = (10,0,5,25)
        self.column_area_interest = pn.Column(\
            pn.pane.Markdown('### Area of Interest',\
                margin = (5,15,5,15)),\
            pn.layout.Divider(height = 5,margin = (-15,5,15,15)),\
            self.visual_controls,\
            pn.layout.Divider(height = 5,margin = (-5,5,5,15)),\
            self.sl_placeholder,\
            height = 350,width = 1150) 
        self.area_image = pn.Tabs(('Spectrum Image',self.column_area_interest),('Saving panel',self.column_saves),\
            height = 400,width = 1150)
        self.image_column = pn.Column(self.area_image,self.spectra_placeholder,\
            width = 1200,height = 750,margin = (0,0,0,50))
