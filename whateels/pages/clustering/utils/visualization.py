...
_CLUSTER_COLORS = ClusterVisualizer.DEFAULT_COLORMAP
_ORDER_COLORS = ClusterVisualizer.DEFAULT_ORDER


def build_cluster_colors_and_scale(
    n_clusters: int,
    colormap: str = _CLUSTER_COLORS,
    index_order: list[int] | None = None
) -> tuple[list[str], list[list]]:
    """Legacy function wrapper for backwards compatibility."""
    visualizer = ClusterVisualizer(n_clusters, colormap, index_order)
    return visualizer.cluster_colors, visualizer.discrete_colorscale


def build_colorbar(n_clusters: int, ny: int, nx: int) -> tuple[dict, dict]:
    """Legacy function wrapper for backwards compatibility."""
    visualizer = ClusterVisualizer(n_clusters)
    return visualizer._build_colorbar(ny, nx)
...
def plot_cluster_labels(
    ...
    visualizer = ClusterVisualizer(n_clusters, colormap, index_order)
    return visualizer.plot_labels(labels, title, height)
...
def plot_cluster_centers(
    ...
    visualizer = ClusterVisualizer(n_clusters)
    # Override colors with provided ones
    visualizer.cluster_colors = cluster_colors
    return visualizer.plot_centers(centres, energy, title)
