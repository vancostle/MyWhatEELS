...
def gos_reader(element,subshell,directory = 'Hartree_Xsections'):
    """
    Read GOS data from JSON files.
    
    Args:
        element: Element symbol (e.g., 'Al', 'Fe')
        subshell: Subshell identifier (e.g., 'L3', 'K')
        directory: Directory name containing GOS JSON files
        
    Returns:
        tuple: (qaxis, energy_axis, gos_array, onset_energy)
    """
    import os
    import json
    ...
    # Go up one level to nlls_library directory
    nlls_library_dir = os.path.dirname(current_dir)
    # Construct path to the data directory
    data_directory = os.path.join(nlls_library_dir, directory)
    
    print(f"GOS Reader: Looking for {element}_{subshell}.json in {data_directory}")
    
    json_file = os.path.join(data_directory, f'{element}_{subshell}.json')
    
    if not os.path.exists(json_file):
        raise FileNotFoundError(
            f"GOS data file not found: {json_file}\n"
            f"Looking for: {element}_{subshell}.json in {data_directory}"
        )
    ...
    try:
        ...
        print(f"  - Successfully loaded GOS data for {element}-{subshell}")
        return (np.array(dictionario['qaxis']), 
                np.array(dictionario['Eaxis']), 
                np.array(dictionario['GOS']), 
                dictionario['onset'])
    except Exception as e:
        raise RuntimeError(f"Failed to load GOS data from {json_file}: {e}")
