...
#Tools from WhatEELS NLLS library
...
class bethe_surface():
    ...
    def __init__(self,element = 'C',verbose = False):
        ...
        verbose = bool()
                ...
                forward
        '''
        print(f"bethe_surface.__init__: Initializing for element '{element}'")
        ...
        if self.element  not in elements.keys():
            ...
            raise KeyError(f"Element '{element}' not found in GOS database")

        else:
            #Let's search for the possible subshells
            self.subshells = list(elements[self.element]['Atomic_properties']\
            ['Binding_energies'].keys())
            print(f"bethe_surface: Element '{self.element}' has subshells: {self.subshells}")
    ...
    def autorun_gosSurfaces(self,exp_param, veto_sshell_list = [],Erange = 100.,mesh_p = 250,*args,**kwargs):
        if not all([True if el in self.\
            ...
            error_msg = ''.join([str00,str01,str02]).format(self.subshells, veto_sshell_list)
            print(error_msg)
            raise NameError(f"Invalid subshell in veto list. Available: {self.subshells}, Given: {veto_sshell_list}")
    ...
    def gos_readout(self,*args,**kwargs):
        ...
        print(f"bethe_surface.gos_readout: Reading GOS for element '{self.element}'")
        print(f"  - Subshells to read: {self.subshells}")
        ...
        for sshell in self.subshells:
            print(f"  - Reading GOS for {self.element}-{sshell}...")
