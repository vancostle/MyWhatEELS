...
### T55 — Retirada del código muerto acumulado en la fase Elemental

Estado: completada. Barrido estático sobre los once commits que van de «New maquetation in Fitting» a «Add DragGutter component for split panes». Cada símbolo retirado se buscó en todo el repositorio —producción, pruebas y documentación— y aparecía exactamente una vez: su propia definición.

- Imports sin uso: `os` en `nlls/multifit.py`, `ModelComposition` en `nlls_controller.py` y `xarray` en el `spectrum_image_plot.py` de Fitting.
- `nlls/cross_sections/protocol.py` eliminado por completo. `CrossSectionProvider` no se importaba en ningún sitio, no figuraba en el `__all__` del paquete y `OOSContinuumProvider` no lo declara como base, de modo que el `Protocol` no restringía nada.
- Métodos muertos: `WhiteLineService.available_components`, `NLLSWorkspace.set_reference_strategy`, `NLLSController._reference_for_active_area`, `NLLSDerivedResultsControlsModal.clear_controls` y las cadenas `select_from_roi`/`from_roi` y `select_default_central`/`default_central` de `ReferenceSpectrumService` —esas cuatro solo se llamaban entre sí; el controlador pasa por `roi_mask`/`central_mask`/`select_from_mask`.
- Constantes muertas: la lista de estrategias de referencia y los títulos y tooltips de las secciones «Areas» y «Run Setup». La estrategia la decide `NLLSController._reference_selection_for_area` a partir del ROI confirmado, y la selección de áreas se trasladó a `NLLSFitAreasModal`, así que ninguna de las dos es seleccionable por el usuario. Se retira también el TODO que pedía migrar los defaults a `whateels/nlls/defaults.py` «once that module exists»: existe y ya se importa en la cabecera del mismo fichero.
- `FittingView.COMPONENT_EAXIS_THRESHOLD` y `_VALUE` quedaron huérfanas al extraer el sidebar en `ebb18d4`: el código que las leía se trasladó a `FittingRightSidebarLayout`, que declara sus propias copias.
- En un segundo commit, código muerto anterior a esta rama detectado por el mismo barrido: `SplitJs._external_resize_event` (inalcanzable, el despacho enruta `external_resize` a `_dragging_event`), `FittingController._test`, una asignación de `.value` sobrescrita por la línea siguiente, la cadena Plotly `build_discrete_colorscale` → `to_plotly_color` → `ColorLike` de `colormaps.py`, cinco watchers `@param.depends` de `AppState` con cuerpo `pass`, y el import `CSS_ROOT` más `_STRETCH_HEIGHT` y `ELEMENT_EAXIS_THRESHOLD` de `FittingView`. Se deja intacta la inversión de magnitudes entre `COMPONENT_EAXIS_THRESHOLD` y `_VALUE` en controlador y layout: corregirla cambia los límites del slider que ve el usuario y no es limpieza.
- Verificación: 209 líneas eliminadas en 14 ficheros, suite de 75 pruebas sin variación respecto al estado previo y todos los paquetes de páginas importables.

### T56 — Ratio de paneA fijado desde el modelo y relayout local del divisor

Estado: completada, verificada en navegador. Cierra el ratio espacial del plot fuente de Fitting, que T52 y T53 daban por resuelto en el raster pero que la caja exterior seguía deformando.

- Rectificación de T52/T53 para `paneA`. `square_pixel_plot_hook` se retira del plot fuente —sigue en los mapas de run y derived—. El hook fija `aspect_ratio=None` y `sizing_mode='stretch_both'` para delegar en `match_aspect`, y eso anula el `aspect_ratio` que `aspect='equal'` acababa de calcular: el mapa se estiraba a todo el panel. `paneA` deja de sobrescribir `_SPLITJS_SIZES_PANEA`, `_paneA_aspect_options` y `_paneA_overlay_options`, quedando como en Home, Clustering y Quantification. De la herencia solo se aprovecha `match_aspect`, que mantiene cuadrado el píxel del dato dentro de la caja.
- `Current Clustering` deja de asignar `paneA.object` directamente. Ahora fija `_paneA_base_overlay` y llama a `_update_selection_overlay([])`, igual que `ClusteringSpectrumImagePlot._update_clustering_plots`. Al saltarse ese camino se perdían las opciones compartidas: el aspecto y el `_client_hover_gate_hook`.
- La caja exterior se calcula en Python sobre el modelo. `DragGutter` mide el panel en el navegador y envía `{width, height}`; `_apply_pane_ratio` aplica el ajuste de `SplitJs._apply_left_plot_pixel_ratio`, `w = min(ancho_disponible, alto_disponible × ratio)` y `h = w / ratio`, sobre `paneA.width`/`height`. Llena el alto mientras el panel es holgado y cede alto en cuanto el ancho pasa a ser el lado limitante.
- Por qué no las otras dos capas, ambas descartadas tras fallar en navegador. `scale_height` de Panel mide el padre una sola vez y este bloque vive dentro de `_StableAdditiveColumn`, un viewport con `overflow-y: scroll` donde esa medida devuelve cero y no se recupera: `paneA` desaparece. Y un tamaño escrito desde JavaScript lo borra Bokeh en su siguiente resolución de layout, porque `paneA` es un elemento que gobierna Bokeh; pedir el relayout justo después dispara ese borrado. CSS tampoco sirve: `aspect-ratio` se descarta en cuanto ancho y alto son ambos definidos, de modo que al topar `max-width` el navegador aplasta la imagen en lugar de devolver alto.
- `ratio_pane` es una referencia Python normal, nunca un parámetro `Child`, así que `paneA` permanece donde Panel lo montó. Eso levanta la prohibición de `send_msg` que fijó T54 sin recuperar su causa: lo que desprendía ejes y colorbars era el reparentado DOM, no la mensajería. Siguen prohibidos `get_child`, `appendChild`, `removeChild`, `insertBefore`, `dispatchEvent` y `ResizeObserver`.
- La medida se envía también **durante** el arrastre, limitada a una cada 150 ms —la cadencia de `SplitJs`—; soltar, doble clic, montaje y resize de ventana se saltan el límite. Sin eso, `paneA` conserva su tamaño `fixed` mientras el panel se estrecha a su alrededor, se sale por detrás del panel derecho y asoma allí donde el espectro no pinta, que es lo que se percibía como un fondo transparente. `overflow: hidden` en la columna izquierda cubre los fotogramas entre dos reportes.
- Los dos paneles vuelven a resolver su layout tras cada cambio de `flex`, recorriendo el árbol de vistas Bokeh y quedándose con la más externa contenida en cada panel. Se descarta el `window.dispatchEvent(new Event('resize'))` global porque despertaría a todos los plots del stack.
- Un watcher sobre `pane_ratio` reajusta desde la última caja reportada, de modo que cambiar al mapa de energía o a un clustering de otra forma no espera al siguiente arrastre.
- Verificación del ajuste con el fit real: panel `800×600` da `355×592`, llenando el alto; al estrechar a `200×600` da `192×320`, cediendo alto con ratio `0.600`; a `60×600` da `52×87`; y un mapa `80×40` en panel `800×600` da `792×396`, ratio `2.000`. Todas las cajas caben dentro del panel. Suite completa de 77 pruebas correctas y todos los paquetes de páginas importables.
- Las regresiones fijan ahora las tres trampas en las que se cayó: `paneA` nunca en `scale_height`, `align` en el pane y jamás en la columna que lo contiene —Panel lo traduce a `align-self`, que en una `Row` es el eje vertical y deja la columna a altura de contenido—, y reporte de geometría dentro del manejador de `pointermove`, no solo al soltar.

### T57 — Flujo final reactivo de DragGutter en Fitting

Estado: completada. Sustituye la cadencia y el doble relayout descritos en T56; no cambia la lógica de ratio ni la topología nativa establecida en T51–T56.

- Los eventos `pointermove` ya no realizan trabajo inmediatamente. Solo conservan la posición más reciente y un único `requestAnimationFrame` aplica el `flex`, descartando posiciones intermedias que el navegador no llegaría a pintar.
- `invalidate_layout()` propaga la invalidación hasta la raíz Bokeh común. Por tanto, recalcular una vista de cada panel resolvía dos veces la misma raíz. El gutter localiza y conserva una sola vista representativa del split y limita el relayout durante el gesto a una pasada cada 50 ms como máximo.
- Reference Fit deja de enviar geometría periódica a Python durante el arrastre. El mapa ratio se identifica mediante `whateels-ratio-pane` y el mismo ajuste de T56 se aplica directamente a su modelo Bokeh del navegador con un único `setv({sizing_mode, width, height}, {sync: false})`. El cambio local invalida por sí mismo la raíz y evita tanto el viaje websocket como un segundo `solve_again`.
- Al soltar se aplica primero la última posición pendiente, se cancela cualquier frame intermedio de drag o relayout y se mide una sola vez la caja exacta. Esa geometría final sí se envía a Python para conservarla ante cambios posteriores de ratio o reemplazos del plot, seguida de una única pasada final de layout. No queda trabajo programado que continúe moviendo el plot después del `pointerup`.
- La persistencia Python se agrupa con `pane.param.update(sizing_mode='fixed', width=..., height=...)`. Se retiraron `min_width`, `max_width`, `min_height` y `max_height`: con `sizing_mode='fixed'` duplicaban las restricciones y generaban cuatro parches adicionales sin alterar la caja.
- Elemental NLLS mantiene su ruta sin ratio y continúa recalculando localmente durante el drag. Para ocultar el canvas antiguo entre dos pasadas se guarda el `overflow` en línea de ambos paneles, se usa `hidden` solo mientras dura el gesto y se restaura después del relayout final; fuera del drag siguen visibles ejes y colorbars.
